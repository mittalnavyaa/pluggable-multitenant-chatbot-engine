...
const STATUS_MAP = {
  ...
  inactive:   { bg: 'var(--badge-danger-bg)',  color: 'var(--badge-danger-text)',  label: 'Inactive' },
};
...
function FullChart({ item }) {
  ...
  const salesTrend = item.salesTrend || [];
  const n = Math.min(trend.length, DAYS.length);
  if (n < 2) return null;

  const allVals = [...trend, ...salesTrend];
  const max = Math.max(...allVals) || 1;
  const xPos = (i) => PAD.left + (i / (n - 1)) * iW;
  const yPos = (v) => PAD.top + iH - (v / max) * iH;

  const toPath = (series) =>
    series.slice(0, n).map((v, i) => `${i === 0 ? 'M' : 'L'}${xPos(i).toFixed(1)},${yPos(v).toFixed(1)}`).join(' ');

  const toFill = (series) =>
    `${toPath(series)} L${xPos(n - 1)},${PAD.top + iH} L${xPos(0)},${PAD.top + iH} Z`;

  const yTicks = [0, 0.25, 0.5, 0.75, 1];
  const color = item.logoColor || 'var(--color-primary)';
  ...
  return (
    <div style={{ width: '100%', overflowX: 'auto' }}>
      ...
      >
        ...
        {DAYS.slice(0, n).map((day, i) => (
          <text key={day} x={xPos(i)} y={H - 8} textAnchor="middle" fontSize="10" fill="#64748b">{day}</text>
        ))}
        {trend.length > 0 && (
          <>
            <path d={toFill(trend)} fill={color} opacity="0.08" />
            <path d={toPath(trend)} fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
            {trend.slice(0, n).map((v, i) => (
              <circle key={i} cx={xPos(i)} cy={yPos(v)} r="4" fill={color} stroke="#f8fafc" strokeWidth="2" />
            ))}
          </>
        )}
        {salesTrend.length > 0 && (
          <>
            <path d={toFill(salesTrend)} fill="#10b981" opacity="0.08" />
            <path d={toPath(salesTrend)} fill="none" stroke="#10b981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
            {salesTrend.slice(0, n).map((v, i) => (
...
function ReportModal({ item, onClose }) {
  const s = STATUS_MAP[item.status] || STATUS_MAP.active;
  const color = item.logoColor || 'var(--color-primary)';
  return (
    <div
      onClick={onClose}
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          {[
            { label: 'Total Queries', value: (item.queries || 0).toLocaleString(), color },
            { label: 'Sales Leads',   value: item.leads || 0,                       color: '#10b981' },
            ...
            <div key={label} style={{
              ...
              <p style={{ margin: '6px 0 0', fontSize: 26, fontWeight: 700, color: c, lineHeight: 1 }}>{value}</p>
            </div>
          ))}
        </div>
        ...
        {item.salesTrend && item.salesTrend.length > 0 && (
          <div style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 12, padding: 16 }}>
            <p style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 600, color: 'var(--color-text)' }}>
              ...
              {item.salesTrend.map((v, i) => {
                ...
                return (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <span style={{ fontSize: 10, fontWeight: 600, color }}>{v}</span>
                    <div style={{
  ...
  return (
    <article className="product-card">
      <div className="product-card__header">
        <span className="product-card__logo" style={{ background: color }}>
          {item.logoInitials}
        </span>
        <div className="product-card__meta">
          <h3 className="product-card__name">{item.product}</h3>
          <p className="product-card__subtitle">{(item.queries || 0).toLocaleString()} queries this week</p>
        </div>
        <span className="status-badge" style={{ background: s.bg, color: s.color }}>{s.label}</span>
      </div>

      <div className="product-card__preview" style={{ paddingBottom: 0 }}>
        <MiniChart trend={item.trend} color={color} />
      </div>

      <dl className="product-card__info">
        <div className="product-card__info-row">
          <dt>Sales Leads</dt>
          <dd style={{ fontWeight: 700, color }}>{item.leads || 0}</dd>
        </div>
        <div className="product-card__info-row">
          <dt>Resolution</dt>
      ...
      try {
        ...
        if (response.ok) {
          ...
          const mapped = data.map((p) => ({
            ...
            logoColor: p.ui_theme_config?.primaryColor || 'var(--color-primary)',
            queries: p.total_queries || 0,
            leads: p.sales_leads || 0,
            resolution: p.resolution_rate ? `${p.resolution_rate}%` : '—',
            trend: p.query_trend || [],
    ...
    <div className="page-stack">
      ...
      {filtered.length === 0 ? (
        <div className="products-empty">
          ...
          <p>{query ? <>No results for <strong>"{query}"</strong></> : 'No sales data available.'}</p>
        </div>
      ) : (
