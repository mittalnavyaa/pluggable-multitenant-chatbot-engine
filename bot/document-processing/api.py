...
def _set_step(job: JobState, step: str, log_line: str) -> None:
    """Advance the job to a new pipeline step and update derived fields."""
    index = PIPELINE_STEPS.index(step) if step in PIPELINE_STEPS else len(PIPELINE_STEPS) - 1
    total = len(PIPELINE_STEPS) - 1          # 'ready' is the last real step
    ...
    for item in job.timeline:
        ...
        elif PIPELINE_STEPS.index(item["step"]) < index:
            item["state"] = "complete"
...
def _build_initial_timeline(file_name: str) -> list[dict[str, Any]]:
    return [
...
def _fail_job(job: JobState, step: str, message: str) -> None:
    """Mark a job as failed at a specific step."""
    job.status = "failed"
    job.progress = 0
...
def _run_pipeline(job_id: str, tmp_path: Path, original_name: str) -> None:
    """Run extraction + LLM sanitization and update job state throughout."""
    with _jobs_lock:
        job = _jobs[job_id]
    ...
    try:
        # ── Validate LLM config before doing any work ────────────────────
        try:
            settings = Settings.from_env()
            if not settings.groq_api_key:
                _fail_job(job, "ai_formatting", "GROQ_API_KEY is not set in .env — open bot/document-processing/.env and add your key.")
                return
        ...
        _set_step(job, "extracting_text", "Text extraction step aligned with PDF, DOCX, and TXT extractors.")
        ...
        if not extraction_result.success:
            ...
            logger.error("extraction_failed", extra={"job_id": job_id, "errors": extraction_result.errors})
            return
        ...
        # ── Step: ai_formatting ──────────────────────────────────────────
        _set_step(job, "ai_formatting", "AI sanitization step aligned with MarkdownSanitizer.")

        writer = MarkdownWriter(_OUTPUT_DIR)
        try:
            sanitizer = MarkdownSanitizer(writer=writer, prompts_dir=_PROMPTS_DIR)
        except LLMAuthenticationError as exc:
            _fail_job(job, "ai_formatting", f"LLM authentication failed: {exc} — check GROQ_API_KEY in .env")
            return
        ...
        if not markdown_result.success:
            ...
            logger.error("sanitization_failed", extra={"job_id": job_id, "errors": markdown_result.errors})
            return
        ...
        _set_step(job, "generating_markdown", "Generating clean Markdown output for downstream chunking.")
        ...
        # ── Step: ready ──────────────────────────────────────────────────
        _set_step(job, "ready", "Markdown output is ready for review and future embedding.")
        job.output_file = md_name

        # Mark all timeline items complete
        for item in job.timeline:
            if item["state"] in ("active", "pending") and item["step"] != "failed":
                item["state"] = "complete"
...
async def submit_upload(file: UploadFile):
    ...
    job = JobState(
        ...
        timeline=_build_initial_timeline(original_name),
    )
    job.logs.append("Upload job accepted by document-processing API.")

    with _jobs_lock:
        _jobs[job_id] = job

    # Use a real OS thread so the pipeline never blocks the async event loop
...
def get_pipeline_status(job_id: str):
    """Return the current pipeline status for a job."""
    job = _get_job(job_id)
    if job is None:
...
def download_output(job_id: str):
    """Stream the finished Markdown file."""
    job = _get_job(job_id)
    if job is None:
        raise HTTPException(status_code=404, detail=f"Job '{job_id}' not found.")
